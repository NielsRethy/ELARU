...
public class Script_BTWindow : EditorWindow {
    ...
    struct HandlesPoint
    {
        ...
        public HandlesPoint(int i, StartOrEnd sOe)
        {
            handleIndex = i;
            startOrEnd = sOe;
        }    
    ...
    struct Handle
    {
        ...
        public Handle(Vector3 s, Vector3 e)
        {
            start = s;
            end = e;
        }
    ...
    Dictionary<int, List<HandlesPoint>> _actionHandlesLink = new Dictionary<int, List<HandlesPoint>>();
    ...
    int _startNode = 0;
    int _endNode = 0;
    List<Handle> _handles = new List<Handle>();
        ...
        if (_BT != null)
        {
            ...
            if (GUILayout.Button("Add Action"))
            {
                ...
                do
                {
                    ...
                    y += (float)(_node.y * 0.2);
                    toAdd = new Rect(x, y, _node.x, _node.y);
                } while (ActionOverlaps(toAdd));
                _actionPositions.Add(toAdd);
                _actionHandlesLink.Add(_actionPositions.Count - 1, new List<HandlesPoint>());
            }
            ...
            if (_showConditions)
            {
                ...
                if (GUILayout.Button("Add Condition"))
                {
                    ...
                    _handles.Add(new Handle(_startPoint, _endPoint));
                    _actionHandlesLink[_startNode].Add(new HandlesPoint(_handles.Count - 1, StartOrEnd.Start));
                    _actionHandlesLink[_endNode].Add(new HandlesPoint(_handles.Count - 1, StartOrEnd.End));
    ...
    private bool ActionAtLocation(Vector2 loc,out int index)
    {
        for (int i = 0; i < _actionPositions.Count; i++)
        {
            if (_actionPositions[i].Contains(loc))
            {
                index = i;
                return true;
            }
    ...
    private bool ActionOverlaps(Rect toCheck)
    {
        foreach (var item in _actionPositions)
        {
            if (item.Overlaps(toCheck)) return true;
        }
        return false;
    }
    ...
    private void Dragging()
    {
            ...
            && _nodeField.Contains(mousePos + _nodeField.position))
        {
            if (button == 0 && _dragging)
            {
                ...
                foreach (var handle in _actionHandlesLink[_selectedAction])
                {
                    if (handle.startOrEnd == StartOrEnd.Start)
                    {
                        var h = _handles[handle.handleIndex];
                        _handles[handle.handleIndex] = new Handle(_offset.center, h.end);
                    }
                    else
                    {
                        var h = _handles[handle.handleIndex];
                        _handles[handle.handleIndex] = new Handle(h.start,_offset.center);
                    }
                }
            }
            else if (button == 1 && !_isStart)
            {
                if (ActionAtLocation(mousePos,out index))
                {
                    _endPoint = mousePos;
                    _endNode = index;
