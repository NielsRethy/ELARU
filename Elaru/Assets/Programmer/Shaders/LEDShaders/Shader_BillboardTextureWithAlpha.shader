Shader "Custom/Shader_BillboardTextureWithAlpha" {
	Properties {
		...
		[MaterialToggle] _PanAlphaWithMainTex("Pan Alpha With Main Texture", Int) = 1
		[MaterialToggle] _UseAlphaTexAlphaChannel("Alpha Is In Alpha Channel", Int) = 0
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) 
		{
			...
			panUV = IN.uv_AlphaTex;
			if (_PanAlphaWithMainTex)
			{
				panUV.x -= frac((_Time + _PanTimeOffset) * _MainTexPanSpeed.x);
				panUV.y -= frac((_Time + _PanTimeOffset) * _MainTexPanSpeed.y);
			}
			fixed4 ac = tex2D(_AlphaTex, panUV);
			float alphaSample = _UseAlphaTexAlphaChannel ? ac.a : ac.r;
			...
			o.Alpha = alphaSample;
			if (_UseEmmissive)
				o.Emission = c * _EmmisiveColor * _EmmissiveStrength * alphaSample;
		}
		ENDCG
	}
	FallBack "Diffuse"
