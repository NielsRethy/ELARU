Shader "Custom/Shader_BillboardTextureAnimated" {
	Properties {
		...
		_FlipSpeed("Texture flip speed", Float) = 0.0
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) 
		{
			...
			float modTime = fmod(_Time.y, _FlipSpeed * 2.f);
			if (modTime > _FlipSpeed)
				c = tex2D(_MainTex2, panUV) * _Color;
