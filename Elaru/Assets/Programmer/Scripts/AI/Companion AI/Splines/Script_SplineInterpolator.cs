...
public class Script_SplineInterpolator : MonoBehaviour
{
	...
	void Update()
	{
	    if (Pauze)
	    {
	        ...
	        if (mState != "Stopped")
	        {
	            ...
	            float param = (mCurrentTime - mNodes[mCurrentIdx].Time) /
	                          (mNodes[mCurrentIdx + 1].Time - mNodes[mCurrentIdx].Time);

	            // Smooth the param
	            param = Script_MathUtils.Ease(param, mNodes[mCurrentIdx].EaseIO.x, mNodes[mCurrentIdx].EaseIO.y);
	...
	Quaternion GetSquad(int idxFirstPoint, float t)
	{
		...
		Quaternion T1 = Script_MathUtils.GetSquadIntermediate(Q0, Q1, Q2);
		Quaternion T2 = Script_MathUtils.GetSquadIntermediate(Q1, Q2, Q3);

		return Script_MathUtils.GetQuatSquad(t, Q1, Q2, T1, T2);
	}
	...
	public Vector3 GetHermiteAtTime(float timeParam)
	{
		...
		param = Script_MathUtils.Ease(param, mNodes[idx].EaseIO.x, mNodes[idx].EaseIO.y);
